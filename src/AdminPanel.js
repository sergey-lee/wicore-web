...
const apiService = {
  ...
  isTokenValid() {
    // Simply check if we have a token
    return !!authToken;
  },

  async ensureAuthenticated() {
    if (!currentUserData || !authToken) {
      console.log('No authenticated user or token:', { hasUserData: !!currentUserData, hasToken: !!authToken });
      throw { code: 'NotAuthenticated', message: 'No authenticated user' };
    }
    return currentUserData;
  },

  // Get user details (without authentication check for initial login)
  async getUserById(userId, skipAuthCheck = false) {
  ...
  async signIn(email, password) {
    try {
      ...
      if (response.data && response.data.accessToken) {
        ...
        // Try to fetch additional user details with retry mechanism
        let userData = null;
        let userDetailsFetched = false;
        
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            console.log(`Fetching user details for username: ${username} (attempt ${attempt}/3)`);
            
            // Add progressive delay for each retry attempt
            if (attempt > 1) {
              await new Promise(resolve => setTimeout(resolve, attempt * 300));
            }
            
            // Skip auth check since we're in the login process
            const userDetailsResponse = await this.getUserById(username, true);
            console.log('User details response:', userDetailsResponse);
            
            // Extract user data from response
            userData = userDetailsResponse.data || userDetailsResponse;
            userDetailsFetched = true;
            console.log(`Successfully fetched user details on attempt ${attempt}`);
            break;
            
          } catch (userError) {
            console.warn(`Failed to fetch user details on attempt ${attempt}:`, userError);
            
            if (attempt === 3) {
              console.warn('All 3 attempts failed, will use basic auth data');
            } else {
              console.log(`Retrying in ${(attempt + 1) * 300}ms...`);
            }
          }
        }
        
        // Build user data based on whether we successfully fetched details
        if (userDetailsFetched && userData) {
          currentUserData = {
            id: username,
            email: email,
          ...
          console.log('Using fetched user details:', currentUserData);
        } else {
          ...
          currentUserData = {
            ...
            isCompany: false, // Default company status
            username: username,
            tokenInfo: tokenInfo
          };
          console.log('Using fallback basic user data after retries failed:', currentUserData);
        }
        ...
        // Check if the auth response itself contains user info
        let isAdminFromAuth = false;
        if (typeof response.isAdmin !== 'undefined') {
          isAdminFromAuth = response.isAdmin;
          console.log('Found isAdmin in auth response:', isAdminFromAuth);
        }
        ...
        for (let attempt = 1; attempt <= 3; attempt++) {
          try {
            console.log(`Fetching user details for username: ${response.username} (attempt ${attempt}/3)`);
            ...
            // Skip auth check since we're in the login process
            const userDetailsResponse = await this.getUserById(response.username, true);
            console.log('User details response:', userDetailsResponse);
        ...
        if (userDetailsFetched && userData) {
          ...
          // Fallback to basic user data (including any auth response data)
          currentUserData = {
            id: response.username,
            email: email,
            name: email.split('@')[0],
            role: isAdminFromAuth ? 'admin' : 'manager',
            isAdmin: isAdminFromAuth,
            isCompany: false, // Default when no details available
            username: response.username,
            accessToken: response.accessToken
          };
          console.log('Using fallback user data after retries failed:', currentUserData);
        }
...
const AuthForm = ({ onAuth }) => {
  ...
  // If in login mode, show login form
  if (isLogin) {
    return (
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                disabled={loading}
              />
              <p className="text-xs text-gray-500 mt-1">Korean phone number required (+82...)</p>
            </div>
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                ...
                disabled={loading}
              />
              <p className="text-xs text-gray-500 mt-1">Enter the code sent to {formData.phoneNumber}</p>
            </div>
            ...
            <button
              ...
              className="w-full text-orange-600 hover:text-orange-800 py-2 text-sm"
            >
...
const DevicesManagement = ({ currentUser }) => {
  ...
  const renderUserHistory = (value) => {
    if (!Array.isArray(value) || value.length === 0) {
      return <span className="text-gray-500">No history</span>;
    }
    
    const latestHistory = value[value.length - 1]; // Get the most recent entry
    
    return (
      <div className="text-sm">
        {latestHistory.uId ? (
          <div>
            <div className="font-medium text-gray-900">User: {latestHistory.uId}</div>
            {latestHistory.paired && (
              <div className="text-gray-600">Paired: {formatDate(latestHistory.paired)}</div>
            )}
            {latestHistory.expiryDate && (
              <div className="text-gray-600">Expires: {formatDate(latestHistory.expiryDate)}</div>
            )}
          </div>
        ) : (
...
const AdminPanel = () => {
  ...
  const getTabsForUser = () => {
    if (currentUser.isAdmin) {
      ...
      // Regular users (isAdmin = false) see Devices tab
      return [
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                {!currentUser.isAdmin && (
                  <div className="bg-green-50 rounded-lg p-4">
                    <h3 className="font-medium text-green-900 mb-2">User Access</h3>
        ...
        {/* Devices Management (Regular users - isAdmin = false) */}
